...
internal partial class EndfieldGameManager : GameManagerBase
{
    ...
    internal async Task<int> InitAsyncInner(bool forceInit = false, CancellationToken token = default)
    {
        ...
        if (IsInstalled)
            ...
            SharedStatic.InstanceLogger.LogWarning("[Endfield] 未检测到安装，将使用默认版本 1.0.0 进行 API 请求");
        ...
        try
        {
            using var response = await ApiResponseHttpClient!.PostAsJsonAsync(ApiUrl, requestBody,
            ...
            SharedStatic.InstanceLogger.LogWarning(
                $"[Endfield] API 响应 - Action: {_latestGameInfo.Action} (0=无更新, 1=有更新), 最新版本: {_latestGameInfo.Version}");
